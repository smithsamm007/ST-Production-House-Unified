...
test("canonical PostgreSQL adapter and migration runner are used", () => {
  const adapter = createPostgresAdapter({ connectionString: "postgresql://invalid:invalid@127.0.0.1:1/invalid", connectionTimeoutMillis: 1 });
  const runner = new MigrationRunner(adapter);
  assert.equal(typeof adapter.withTransaction, "function");
