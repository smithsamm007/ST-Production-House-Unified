...
describe("Credential Broker - Opaque Locator Contracts [TASK-4.1]", () => {

  test("Round-trip: mintLocator -> parseLocator returns identical id and version v1", () => {
  ...
  test("Tamper tests: empty string, null, undefined, whitespace rejected with EMPTY code", () => {
  ...
  test("Tamper tests: wrong prefix, flipped chars, non-string/invalid structure rejected with MALFORMED code", () => {
  ...
  test("Tamper tests: loc_v9_... and unsupported version prefix rejected with WRONG_VERSION code", () => {
  ...
  test("Uniqueness: 10,000 mints produce 10,000 distinct ids", () => {
    const set = new Set();
    const count = 10000;
  ...
  test("No secret leak test: locator contains NO provider name, NO agent name, NO secret", () => {
  ...
  test("isValidLocator: never throws under any condition", () => {
    const hostileInputs = [
