...
class MockDatabase {
  ...
  async query(text, params = []) {
    ...
    if (normalized.startsWith("INSERT INTO jobs")) {
      ...
      if (existing) {
        // ON CONFLICT DO NOTHING (simulate zero rows returned, then select)
    ...
    if (normalized.includes("SELECT id FROM jobs") && normalized.includes("status = 'queued'")) {
      ...
      const queuedJobs = this.jobs
        .filter((j) => j.agent_id === agentId && j.capability === capability && j.status === "queued")
        .sort((a, b) => {
...
test("Job Lifecycle Contract — Mock/Unit Suite", async (t) => {
  ...
  await t.test("concurrency limit prevents exceeding maximum parallel jobs per agent", async () => {
    // agent-01 has concurrency_limit = 2.
    // Clear jobs
    db.jobs = [];
  ...
  await t.test("lease renewal extends expiry correctly", async () => {
    ...
    // Attempt renew with wrong owner should throw
    await assert.rejects(
      async () => {
        await renewLease(db, { jobId: claimed.id, leaseOwner: "imposter", leaseDurationSeconds: 30 });
      },
  ...
  await t.test("reclaimExpiredLeases handles timed out leases", async () => {
    ...
    const claimed = await claimJob(db, { agentId: "agent-01", capability: "render", leaseOwner: "w-timeout", leaseDurationSeconds: -5 }); // already expired
...
test("Job Lifecycle Contract — Live PG Suite", async (t) => {
  ...
  // Bootstrapping the database for integration test run
  try {
    ...
    await t.test("respects concurrency limits and locks on real PostgreSQL", async () => {
    ...
    await t.test("strict database-level job status transitions trigger verification", async () => {
      ...
      // Try transition from queued directly to running (illegal - queued can only transition to leased)
      await assert.rejects(
        async () => {
      ...
      // Try transition succeeded -> queued (illegal - succeeded is terminal status)
      await assert.rejects(
        async () => {
    ...
    await t.test("reclaims expired leases in real PG", async () => {
      ...
      // Claim with negative duration to expire immediately
