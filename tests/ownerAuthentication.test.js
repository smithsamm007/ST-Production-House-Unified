...
import {
  ...
  useInMemoryOwnerAuthenticationForTests,
  generateTotpCode
...
test.beforeEach(() => useInMemoryOwnerAuthenticationForTests());
...
test("3. Passwords are stored only as Argon2id hashes", async () => {
  const passwordHash = await hashPassword("PasswordSecure123");
  assert.ok(passwordHash.startsWith("$argon2id$v=19$m=65536"));
});
...
test("5. Correct password verification succeeds", async () => {
  const passwordHash = await hashPassword("PasswordSecure123");
  assert.ok(await verifyPassword("PasswordSecure123", passwordHash));
});
...
test("13. MFA completion rotates the session", async () => {
  ...
  await confirmTotpMfa(owner.id, enroll.enrollmentId, generateTotpCode(enroll.secret, Date.now() - 30_000).code);

  // Verify TOTP elevates session
  const result = await verifyTotpAndElevateSession(owner.id, s1.token, generateTotpCode(enroll.secret).code);
...
test("28. Valid TOTP completes MFA", async () => {
  ...
  const enroll = await enrollTotpMfa(owner.id, s1.token);
  const confirm = await confirmTotpMfa(owner.id, enroll.enrollmentId, generateTotpCode(enroll.secret).code);
...
test("29. Replayed TOTP is rejected", async () => {
  ...
  const enroll = await enrollTotpMfa(owner.id, s1.token);
  const code = generateTotpCode(enroll.secret).code;
  await confirmTotpMfa(owner.id, enroll.enrollmentId, code);

  await assert.rejects(async () => {
    await verifyTotpAndElevateSession(owner.id, s1.token, code);
...
test("31. Security audit events contain no secrets", async () => {
  resetOwnerAuthenticationRegistry();
  const owner = await registerOwner("owner@st.com", "PasswordSecure123");
...
test("32. Failed actions do not create success evidence", async () => {
  resetOwnerAuthenticationRegistry();
  const owner = await registerOwner("owner@st.com", "PasswordSecure123");
  await assert.rejects(async () => {
    await loginOwner("owner@st.com", "WrongPassword123");
...
test("33. Task 8 Charter approval remains owner-controlled", async () => {
  resetOwnerAuthenticationRegistry();
  const owner = await registerOwner("owner@st.com", "PasswordSecure123");
...
test("34. Charter approval does not activate autopilot or publishing", async () => {
  resetOwnerAuthenticationRegistry();
  const owner = await registerOwner("owner@st.com", "PasswordSecure123");
  const s1 = await createSessionToken(owner.id, "high_assurance");
