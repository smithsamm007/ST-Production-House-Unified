...
let repositoryAdapter = null;

export function configureRepositoryAdapter(adapter) {
  if (!adapter || typeof adapter.query !== "function" || typeof adapter.withTransaction !== "function") {
    throw new Error("VALID_POSTGRES_ADAPTER_REQUIRED");
  }
  repositoryAdapter = adapter;
}

async function query(text, params = []) {
  if (!repositoryAdapter) throw new Error("POSTGRES_REPOSITORY_NOT_CONFIGURED");
  return repositoryAdapter.query(text, params);
}

async function transaction(callback) {
  if (!repositoryAdapter) throw new Error("POSTGRES_REPOSITORY_NOT_CONFIGURED");
  return repositoryAdapter.withTransaction(callback);
}
...
export class OwnerRepository {
  async createInitial(owner) {
    return transaction(async (client) => {
      await client.query("SELECT pg_advisory_xact_lock($1, $2)", [72101, 1]);
      const count = await client.query("SELECT count(*)::integer AS count FROM owners");
      if (Number(count.rows[0].count) !== 0) throw new Error("OWNER_BOOTSTRAP_ALREADY_COMPLETED");
      const result = await client.query(
        `INSERT INTO owners
           (id, email, password_hash, role, status, mfa_enabled, password_changed_at, session_revocation_epoch)
         VALUES ($1, $2, $3, 'owner', $4, false, $5, 1) RETURNING *;`,
        [owner.id || randomUUID(), owner.email, owner.passwordHash, owner.status || "anonymous", owner.passwordChangedAt]
      );
  ...
  async recordLoginFailure(ownerId) {
    const result = await query(
      `UPDATE owners
       SET failed_login_attempts = failed_login_attempts + 1,
           lockout_until = CASE WHEN failed_login_attempts + 1 >= 5
             THEN now() + interval '15 minutes' ELSE lockout_until END,
           status = CASE WHEN failed_login_attempts + 1 >= 5 THEN 'locked' ELSE status END,
           updated_at = now()
       WHERE id = $1 RETURNING failed_login_attempts, lockout_until;`,
      [ownerId]
    );
    return result.rows[0] || null;
  }

  async recordLoginSuccess(ownerId, status) {
    const result = await query(
      `UPDATE owners SET failed_login_attempts = 0, lockout_until = NULL,
         last_success_at = now(), status = $2, updated_at = now()
       WHERE id = $1 RETURNING *;`,
      [ownerId, status]
    );
    return result.rows[0];
...
export class SessionRepository {
  ...
  async revoke(id, ownerId) {
    const result = await query(
      "UPDATE owner_sessions SET revoked_at = now() WHERE id = $1 AND owner_id = $2 AND revoked_at IS NULL RETURNING id;",
      [id, ownerId]
    );
...
export class MfaRepository {
  async createTotpEnrollment(enrollment) {
    const res = await query(
      `INSERT INTO owner_totp_enrollments
         (id, owner_id, encrypted_totp_secret, is_confirmed, last_used_step, created_at, updated_at)
  ...
  async confirmTotpEnrollment(id, ownerId, usedStep) {
    await transaction(async (client) => {
      const enrollment = await client.query(
        `UPDATE owner_totp_enrollments
         SET is_confirmed = true, last_used_step = $3, updated_at = now()
         WHERE id = $1 AND owner_id = $2 AND is_confirmed = false RETURNING id;`,
        [id, ownerId, usedStep]
      );
      if (enrollment.rowCount !== 1) throw new Error("MFA_ENROLLMENT_NOT_FOUND");
      await client.query("UPDATE owners SET mfa_enabled = true, status = 'authenticated', updated_at = now() WHERE id = $1;", [ownerId]);
    });
  }

  async consumeTotpStep(id, ownerId, usedStep) {
    const result = await query(
      `UPDATE owner_totp_enrollments SET last_used_step = $3, updated_at = now()
       WHERE id = $1 AND owner_id = $2 AND is_confirmed = true
  ...
  async verifyAndUseRecoveryCode(ownerId, codeHash) {
    const res = await query(
      `UPDATE owner_recovery_codes SET is_used = true, used_at = now()
       WHERE owner_id = $1 AND code_hash = $2 AND is_used = false RETURNING id;`,
      [ownerId, codeHash]
    );
...
export class CsrfRepository {
  async createToken(sessionId, tokenValue) {
    ...
    await query(
      `INSERT INTO csrf_session_tokens (session_id, token_hash)
       VALUES ($1, $2)
       ON CONFLICT (session_id) DO UPDATE SET token_hash = EXCLUDED.token_hash, created_at = now();`,
      [sessionId, tokenHash]
    );
  }
  ...
  async verifyToken(sessionId, tokenValue) {
    ...
    const stored = Buffer.from(res.rows[0].token_hash, "hex");
