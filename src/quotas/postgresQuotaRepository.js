...
export class PostgresQuotaRepository {
  ...
  async #finalize(reservation, targetStatus) {
    ...
    return this.adapter.withTransaction(async (client) => {
      // 1. Fetch reservation without locking to obtain quota_id
      ...
      const quotaId = precheck.rows[0].quota_id;

      // 2. Lock provider_quota_limits FIRST to prevent deadlock (same order as reserve)
      await client.query(
      ...
      // 3. Lock provider_quota_reservations SECOND and get the rest of the details
      const result = await client.query(
